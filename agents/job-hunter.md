...
**Empty page handling**: If `dom_getText` returns fewer than 100 characters of useful job content (not counting boilerplate text), the page likely requires JavaScript that hasn't loaded. Do not retry the same board unless it is the first board and no listings were extracted. Do not visit the same board more than 2 times total. If the page has fewer than 100 words of useful content, shows a login page, returns empty search results, or appears to have failed to load JavaScript, prioritize moving to the next job board instead of trying additional keywords. If 2 consecutive job boards produce no valid results, stop searching and report the boards tried along with suggested, more precise keywords for the user to try.
